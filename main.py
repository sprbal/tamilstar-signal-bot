...
GMAIL_PASS = "prmrddzbvdjwsdvv"  # Spaces removed
...
def process_tradingview_alert(body_text, subject_text):
    ...
    # Identify Asset
    asset = "EUR/USD"
    for cur in ["EURUSD", "GBPUSD", "USDJPY", "AUDUSD", "USDCAD", "EURGBP", "EURJPY", "GBPJPY", "XAUUSD"]:
        if cur in combined:
            if cur == "XAUUSD":
                asset = "GOLD (XAU/USD)"
            else:
...
def check_gmail_loop():
    print("TamilStar Gmail Real-Time TradingView Engine Active...")
    get_channel_id()
    while True:
        try:
