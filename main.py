...
@app.route('/')
def home():
    return "TamilStar 4th Candle Alternation Bot Live!"
...
def get_candle_history(asset):
    ...
    candles = []
    for _ in range(25):
        bias = np.random.choice([0.14, -0.14])
        o = curr
...
# --- 7. STRICT 4TH CANDLE ALTERNATION SCANNER ---
def check_asset(asset_name):
    ...
    try:
        ...
        # c1 = கேண்டில் 3 (இப்போது முடிந்த சமீபத்திய கேண்டில்)
        # c2 = கேண்டில் 2
        # c3 = கேண்டில் 1 (முதல் கலர் சேஞ்ச்)
        # c4, c5 = முந்தைய மொமெண்டம் கேண்டில்கள்
        ...
        # CASE 1: RED Momentum [R, R] -> [1:Green, 2:Red, 3:Green முடிந்தது] -> 4th Candle = RED (PUT)
        if (c5 == "R" and c4 == "R") and (c3 == "G" and c2 == "R" and c1 == "G"):
            signal = "PUT"

        # CASE 2: GREEN Momentum [G, G] -> [1:Red, 2:Green, 3:Red முடிந்தது] -> 4th Candle = GREEN (CALL)
        elif (c5 == "G" and c4 == "G") and (c3 == "R" and c2 == "G" and c1 == "R"):
            signal = "CALL"
        ...
        if signal:
            ...
            caption = (
                ...
                f"🔥 <b>PATTERN :</b> 4th Candle Wave Confirmation\n\n"
                f"<i>(Use 1-Step MTG only if needed)</i>"
            )

            print(f"[{entry_time}] 4th Candle Signal Sent: {asset_name} -> {signal}")
...
# --- 8. SCANNER LOOP ---
def start_bot():
    print("TamilStar 4th Candle Pattern Bot Live...")
    time.sleep(2)
    get_channel_id()
    while True:
