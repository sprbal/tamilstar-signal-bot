...
# ==================== BOT CONFIGURATIONS ====================
TELEGRAM_BOT_TOKEN = "8837639367:AAEQTwjnI9Ed9Pg9Ln3r6GSIWljnSICIzG8"
# பப்ளிக் சேனல் யூசர்நேம் (நேரடி டெலிவரி)
...
def send_telegram_message(text):
    ...
    try:
        ...
        # ஒருவேளை யூசர்நேமில் பிரச்சனை இருந்தால் ID மூலமாக அனுப்ப பேக்கப்
        if not res.json().get("ok"):
            payload["chat_id"] = "-1004312613865"
            res = requests.post(url, json=payload, timeout=10)
            logging.info(f"Fallback ID Response: {res.text}")
        return res.json()
